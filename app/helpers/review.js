module.exports.getReviewQuestions = function(request, response, db){
  ...
  db.query('SELECT CONVERT(QuestionText using UTF8) as QuestionText, QuestionAnswer, review_session, streak FROM Questions JOIN Student_Questions\
    ON Student_Questions.id_Questions = Questions.id JOIN Users On Student_Questions.id_Students = Users.id\
    WHERE Student_Questions.id_Students = ? ORDER BY\
